...
from keras.layers import Activation, Conv2D, MaxPooling2D, Flatten, Dense, BatchNormalization
...
model.summary()
model.compile(optimizer=Adam(lr=0.0005), loss='categorical_crossentropy', metrics=['accuracy', Precision(), Recall()])
...
datagen = ImageDataGenerator(
    ...
    vertical_flip=False,
    zoom_range=[0.5, 1.3],  # ランダムに画像をズームする範囲
    shear_range=0.25  # シアー強度（反時計回りのシアー角度（ラジアン））    
)
